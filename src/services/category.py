...
class CategoryService:
    ...
    def change_category(self, category: Category) -> Category:  # TODO рефакторинг нужен
        ...
        if (category.parent_id != -1 and category.parent_id is not None
                and self.__get_category_model_by_id(category.parent_id) is None):
        ...
        return Category(name=category_row.name, id=category_row.id, parent_id=parent)
    ...
    def __get_top_level_category_ids(self) -> List[int]:
        ...
        categories = self.session.query(CategoryModel)\
            .join(LevelCategoryModel, (and_(CategoryModel.id == LevelCategoryModel.children_id)))\
            .filter(and_(CategoryModel.user_id == self.user_id, LevelCategoryModel.parent_id.is_(None))).all()
    ...
    def __get_category_tree(self, category_id) -> Optional[CategoryTree]:
        ...
        """
        children = self.session.query(LevelCategoryModel).filter(LevelCategoryModel.parent_id == category_id).all()
        tree = CategoryTree(name=self.__get_category_model_by_id(category_id).name, id=category_id, children=[])
    ...
    def __change_category_name(self, category_id: int, new_name: str) -> CategoryModel:
        """
        ...
        :return: modified category
        """
        category = self.session.query(CategoryModel).filter(and_(CategoryModel.id == category_id,
                                                                 CategoryModel.user_id == self.user_id)).first()
        category.name = new_name
        self.session.commit()
        return category

    def __change_category_parent_id(self, child_id: int, new_parent_id: int) -> LevelCategoryModel:
        """
        ...
        changed_level = (self.session.query(LevelCategoryModel)
                         .filter(LevelCategoryModel.children_id == child_id).first())
