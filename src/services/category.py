...
class CategoryService:
    ...
    def __delete_category_tree(self, tree: Optional[CategoryTree]) -> NoReturn:
        ...
        self.session.query(CategoryModel).filter(and_(CategoryModel.user_id == self.user_id,
                                                      CategoryModel.id == tree.id)).delete()
        self.session.commit()
    ...
    def __change_category_name(self, id: int, new_name: str) -> CategoryModel:
        """
        ...
        :return: modified category
        """
        category = self.session.query(CategoryModel).filter(and_(CategoryModel.id == id,
                                                                 CategoryModel.user_id == self.user_id)).first()
